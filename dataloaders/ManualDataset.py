...
class ManualDataset(BaseDataset):
    def __init__(self, which_dataset, pids, base_dir, mode, transforms, scan_per_load, min_fg = '', fix_length = None, tile_z_dim = 3, nsup = 1, exclude_list = [], extern_normalize_func = None,unlbl_pids = None ,test_labels = None,**kwargs):
        """
        Manually labeled dataset
        Args:
            which_dataset:      name of the dataset to use
            pids: (Added)       list of patient IDs to be considered
            unlbl_pids: (Added) list of unlabeled patient IDs to be used for self-supervised training
            test_labels:(Added) list of test labels which are supposed to be unseen while training
            base_dir:           directory of dataset
            idx_split:(Removed) index of data split as we will do cross validation
            mode:               'train', 'val'
        ...
        if pids == None: # consider all images
        ...
        self.scan_ids = self.img_pids # patient ids of the entire fold
        ...
        # Below functions be useful for deciding  +ve, -ve pairs (inter-class)
        self.overall_slice_by_cls = self.read_classfiles() # Stores the {class: {pid: <slice_containing_that_class>}}
    ...
    def read_dataset(self):
        ...
        for scan_id, itm in self.img_lb_fids.items():
            ...
            if len(lb.shape) == 3:
                ...
                lb = lb.transpose(2,3,1,0) # (H,W,num_class,channels)

            lb = np.float32(lb)

            img = img[:256, :256, :] # FIXME a bug in shape from the pre-processing code
            lb = lb[:256, :256, ...] ## label will now be (H,W,num_class,slices)
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            ### Try adding transforms in training also
            if self.transforms:
        ...
        img = np.float32(img)
        lb = np.float32(lb).squeeze(-1) # NOTE: to be suitable for the PANet structure (256,256,1)
        
        img = torch.from_numpy( np.transpose(img, (2, 0, 1)) )
        lb  = torch.from_numpy( lb)
        
        ## REMEBER - We are repeating the 2D slices along channels
        if self.tile_z_dim:
    ...
    def update_subclass_lookup(self):
        """
        Joint self-supervised training : Make sure that test classes only contain slices from unlbl_list
        ...
        # To validate the PIDs used for each class - unseen classes must not have labeled PIDs : Training on pseudo-labels only
        self.debug_cls = {}  
    ...
    def subsets(self, sub_args_lst=None):
        ...
        Self-supervised joint training - Test/Unseen classes should only contain images from the unlbl_list
        Handle this while generating idx_by_class table
    ...
    def get_support(self, curr_class: int, class_idx: list, scan_idx: list, npart: int):
        ...
        for _part in range(npart):
            ...
            for scan_order in scan_idx:
                ...
                # _zlist = self.overall_slice_by_cls[self.label_name[curr_class]][_scan_id] # list of indices
