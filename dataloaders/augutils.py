...
def transform_with_label(aug):
    ...
    def transform(comp, c_label, c_img, use_onehot, nclass, **kwargs):
        ...
        if comp.shape[-1] == 2: # Compact labels
            _label = comp[..., c_img ] 
            # This is converting to one-hot (But we will already convert it and send to this func.)
            _h_label = np.float32(np.arange( nclass ) == (_label[..., None]) )
            comp = np.concatenate( [comp[...,  :c_img ], _h_label], -1 )
            
        
        # Transforms are done on the one-hot encoded part
