...
def getMaskOnly(label, class_id, class_ids):
    ...
    UPDATE : Will be changed, instead of torch.where, just pick the corresponding channel in the class of label
    Args:
        label:
            semantic mask
        scribble:
    ...
    if len(label.shape) > 2: 
        # Pick the corresponding label mask and Make label suitable with PANet architecture
        
        label = label[...,class_id]
        
        fg_mask = label
        
        bg_mask = torch.where(label != 1.0, torch.ones_like(label), torch.zeros_like(label))
...
def fewshot_pairing(paired_sample, n_ways, n_shots, cnt_query, coco=False, mask_only = True):
    ...
    ## DEBUG
    supp_scan_z_ids = [(paired_sample[cumsum_idx[i]]['scan_id'],paired_sample[cumsum_idx[i]]['z_id']) for i in range(n_ways)]
    ...
    query_scan_z_ids = [(paired_sample[cumsum_idx[i+1] - j - 1]['scan_id'],paired_sample[cumsum_idx[i+1] - j - 1]['z_id']) for i in range(n_ways)
                    for j in range(cnt_query[i])]
    ...
    if coco:
        ...
        # before : (256,256) all labels in single image,  after : (256,256,5) 1 channel for each label
        query_labels = [paired_sample[cumsum_idx[i+1] - j - 1]['label'] for i in range(n_ways)
                        for j in range(cnt_query[i])]
    
    
    if(len(query_labels[0].shape) == 2):
        ...
        ## query_label_tmp : Actual GT mask : 1 for class, 0 for bg
        ...
        ## Right now only supports 1 way 1 shot setting
        ## Generate query fg and bg masks just like the support image (By indexing channels)
        # BG class
        query_cls_idx = [0,] + class_ids
        query_labels_tmp = [query_label[:,:,class_ids[0]] for query_label in query_labels]

        query_masks = [[torch.where(query_label[:,:,class_ids[0]] == 0, torch.ones_like(query_label[:,:,class_ids[0]]), torch.zeros_like(query_label[:,:,class_ids[0]]))[None, ...],] for query_label in query_labels]
        # FG class - Only one in 1-w 1-s setting
        for i, query_label in enumerate(query_labels):
            mask = query_label[:,:,class_ids[0]]
            query_masks[i].append(mask)
    ...
    return {'class_ids': class_ids,
            ...
            'support_scribbles': [], 
            'supp_scan_z_ids' : supp_scan_z_ids,
            'query_images': query_images,
            'query_labels': query_labels_tmp,
            'query_masks': query_masks,
            'query_cls_idx': query_cls_idx,
            'query_all_lbls': query_labels[0],
            'query_scan_z_ids': query_scan_z_ids
           }
...
def med_fewshot(dataset_name, base_dir, tr_pids, mode, scan_per_load,
    ...
    ## Testing with specified training images
    med_set = ManualDataset
    ...
    # subsets is a list of slices of images which have the same class (len(subsets) = no_classes) 
    # Useful for training
    ...
    # Based on the subsets, and selected query create a paired dataset
    # For setting up the few-shot training - each class needs to have its support and query
