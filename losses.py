...
def local_cont_loss(y_fin,y_l_reg,pos_indx,neg_indx,dataset,num_filters=16,batch_size=1,local_loss_exp_no=0):
    """
    ...
    Inputs:
        ...
        y_l_reg : Labels for the image (H,W,num_classes) - check this; pos/neg indices have num_class - 1 (no background), y_l_reg has BG (256,256,6)
    ...
    if dataset == 'CHAOST2':
        ...
        pos_cls_ref = np.asarray([0,1,2,5])
        neg_cls_ref = np.asarray([1,2,3,6])
    elif dataset == 'C0':
        ...
        for pos_cls in pos_cls_ref:
            ...
            for n_pos_idx in range(pos_indx.shape[-2]):
                ...
                if (x_coord_i1 == 1000 or y_coord_i1==1000):
                    # print("Image at pos {}, skip".format(index_pos1))
                    continue
                ...
                if (x_coord_i2 == 1000 or x_coord_i2==1000):
                    # print("Image at pos {}, skip".format(index_pos2))
                    continue
                ...
                if local_loss_exp_no == 1:
                    ## Image 1 and 2
                    log_or_i1_i2 = (torch.equal(torch.count_nonzero(x_num_tmp_i1),torch.tensor(0).cuda()) or torch.equal(torch.count_nonzero(pos_avg_vec_i2),torch.tensor(0).cuda()))
                    log_or_i1_i2_nan = bool(torch.logical_or(torch.isnan(torch.sum(x_w3_n_i1)),torch.isnan(torch.sum(pos_avg_vec_i2))))
                    log_or_i1_i2_net = (log_or_i1_i2 or log_or_i1_i2_nan)
                    if log_or_i1_i2_net:
                        num_i1_i2_ss = torch.tensor(0.0).cuda()
                    else:
                        num_i1_i2_ss = cos_sim(x_w3_n_i1,pos_avg_vec_i2)

                    ## Image 2 and 1
                    log_or_i2_i1 = (torch.equal(torch.count_nonzero(x_num_tmp_i2),torch.tensor(0).cuda()) or torch.equal(torch.count_nonzero(pos_avg_vec_i1),torch.tensor(0).cuda()))
                    log_or_i2_i1_nan = bool(torch.logical_or(torch.isnan(torch.sum(x_w3_n_i2)),torch.isnan(torch.sum(pos_avg_vec_i1))))
                    log_or_i2_i1_net = (log_or_i2_i1 or log_or_i2_i1_nan)
                    if log_or_i2_i1_net:
                        num_i2_i1_ss = torch.tensor(0.0).cuda()
                    else:
                        num_i2_i1_ss = cos_sim(x_w3_n_i2,pos_avg_vec_i1)
                ...
                if local_loss_exp_no == 1:
                    for neg_avg_i1_c2 in neg_mask2_list:
                        log_or_n1_d2 = (torch.equal(torch.count_nonzero(x_num_tmp_i1),torch.tensor(0).cuda()) or torch.equal(torch.count_nonzero(neg_avg_i1_c2),torch.tensor(0).cuda()))
                        log_or_n1_d2_nan = bool(torch.logical_or(torch.isnan(torch.sum(x_w3_n_i1)),torch.isnan(torch.sum(neg_avg_i1_c2))))

                        log_or_n1_d2_net = (log_or_n1_d2 or log_or_n1_d2_nan)
                        if log_or_n1_d2_net:
                            den_i1_ss = den_i1_ss + torch.tensor(0.0).cuda()
                        else:
                            tmp = torch.exp(cos_sim(x_w3_n_i1,neg_avg_i1_c2)) 
                            den_i1_ss = den_i1_ss + tmp
                ...
                if local_loss_exp_no == 1:
                    for neg_avg_i2_c1 in neg_mask1_list:
                        log_or_n2_d1 = (torch.equal(torch.count_nonzero(x_num_tmp_i2),torch.tensor(0).cuda()) or torch.equal(torch.count_nonzero(neg_avg_i2_c1),torch.tensor(0).cuda()))
                        log_or_n2_d1_nan = bool(torch.logical_or(torch.isnan(torch.sum(x_w3_n_i2)),torch.isnan(torch.sum(neg_avg_i2_c1))))

                        log_or_n2_d1_net = (log_or_n2_d1 or log_or_n2_d1_nan)
                        if log_or_n2_d1_net:
                            den_i2_ss = den_i2_ss + torch.tensor(0.0).cuda()
                        else:
                            den_i2_ss = den_i2_ss + torch.exp(cos_sim(x_w3_n_i2,neg_avg_i2_c1))
                ...
                if local_loss_exp_no == 1:
                    log_num_i1_i2_nan = bool(torch.logical_or(torch.isnan(torch.exp(num_i1_i2_ss)),torch.isnan(torch.exp(den_i1_ss))))

                    log_num_i1_i2_zero = (torch.equal(torch.count_nonzero(num_i1_i2_ss),torch.tensor(0).cuda()) or torch.equal(torch.count_nonzero(den_i1_ss),torch.tensor(0).cuda()))
                    log_num_i1_i2_net = (log_num_i1_i2_nan or log_num_i1_i2_zero)

                    if log_num_i1_i2_net:
                        local_loss = local_loss + torch.tensor(0.0).cuda()
                    else:
                        local_loss = local_loss - torch.log( torch.exp(num_i1_i2_ss) / den_i1_ss)

                    log_num_i2_i1_nan = bool(torch.logical_or(torch.isnan(torch.exp(num_i2_i1_ss)),torch.isnan(torch.exp(den_i2_ss))))

                    log_num_i2_i1_zero = (torch.equal(torch.count_nonzero(num_i2_i1_ss),torch.tensor(0).cuda()) or torch.equal(torch.count_nonzero(den_i2_ss),torch.tensor(0).cuda()))
                    log_num_i2_i1_net = (log_num_i2_i1_nan or log_num_i2_i1_zero)

                    if log_num_i2_i1_net:
                        local_loss = local_loss + torch.tensor(0.0).cuda()
                    else:
                        local_loss = local_loss - torch.log( torch.exp(num_i2_i1_ss) / den_i2_ss)


        if local_loss_exp_no == 1:
            local_loss = local_loss/ (2*num_pos_elem*(num_classes))
        else:
